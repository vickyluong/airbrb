...
import { Rating, Button, Dialog, DialogTitle, DialogContent, DialogActions, TextField } from '@mui/material';
...
function MyBookings(props) {
  ...
  // function which handles leaving a review 
  ...
  useEffect(() => {
    // function which returns bookings with the listing's details
      async function fetchDetails() {
        const userBookings = bookings.filter(booking => booking.owner === user);
  
        const detailed = await Promise.all(
          userBookings.map(async booking => {
            const response = await api.getListingDetails(token, booking.listingId);

            return { ...booking, listing: response.listing }; 
          })
        );
